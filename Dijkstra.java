...
public class Dijkstra {
  private LinkedList<Vertex> kVertices;
  ...
  public Dijkstra() {
      ...
      kVertices = new LinkedList<Vertex>();
      minSofar = Double.POSITIVE_INFINITY;
  ...
  public double computeEuclideanDistance(double ux, double uy, double vx, double vy) {
        // TODO
      double k = Math.sqrt(Math.pow(vx-ux, 2)+(Math.pow(vy-uy, 2)));
      return k;
  }
  ...
  public void doDijkstra(String s) {
      ...
      for(Vertex h: getVertices()){ 
          h.distance = Double.POSITIVE_INFINITY;
          h.known = false;
          kVertices.add(h);
      }   
      
      Vertex st = getVertex(s);
      st.distance = 0.0;
      Vertex v;

      while(!kVertices.isEmpty()){
          ...
          for(Edge e: v.adjacentEdges){
                  Vertex t = e.target;
                  if(!t.known){
                      double cvw = computeEuclideanDistance(t.x, t.y, v.x, v.y);
                      if(v.distance + cvw < t.distance){
                          t.distance = v.distance + cvw;
                          t.prev = v;//this is the path that tells me the vertex before me
                      }
                  }
          }
      }
  ...
  //Shortest path: 
  //San Fran -> Salt Lake City -> Denver -> Omaha -> Chicago -> Pittsburgh -> New York -> Boston
   // get the smallest vertex.distance and remove the vertex from known vertices 
   private Vertex getMin(LinkedList<Vertex> k){
       
       String minVert = "";
       
       for(Vertex j: k){
           if(j.distance<=minSofar){ 
              minSofar = j.distance;
              minVert = j.name;
           }
       }
       ...
       minSofar = Double.POSITIVE_INFINITY;
       Vertex v = getVertex(minVert);
       return getVertex(minVert);
   }
  ...
  public List<Edge> getDijkstraPath(String s, String t) {
    ...
    // TODO
      ...
      LinkedList<Edge> pathEdgesTemp = new LinkedList<Edge>();
      Vertex a = getVertex(s);
      Vertex c = getVertex(t);
      
      for (int i = 0; i<getVertices().size();i++){
          if(c.prev!=a){
              Vertex b = c.prev;
              Edge k = new Edge(c.prev, c, computeEuclideanDistance(c.x, c.y, b.x, b.y));
              pathEdgesTemp.add(k);
              c.prev = b.prev;
              c = b;
          }
          if(i==getVertices().size()-1){
              Edge l = new Edge(a, c, computeEuclideanDistance(c.x, c.y, a.x, a.y));
              pathEdgesTemp.add(l);
          }
      }
      
      //reverse the list of edges 
      for(int i = pathEdgesTemp.size()-1;i>=0;--i){
          pathEdges.add(pathEdgesTemp.get(i));
      }      
    return pathEdges; 
  }
